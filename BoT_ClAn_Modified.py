...
def  generate_random_word():
    word_list = [
        ...
        "يافاشل", "نيكمك", "كوسمك", "يكسمك", "هضرب_عشره_عليك", "لحاااااس", " FOXXX", "TELEGRAM:"
    ]
...
class FF_CLIENT(threading.Thread):
    ...
    def GenResponsMsg(self, Msg):
        ...
        1: 1,
        2: {
            1: 11497872667,#bot account id
            2: 3068617405,#clan id
            3: 1,
            4: str(Msg),
    ...
    def joinclanchat(self):
        fields = {
            1: 3,
            2: {
                1: 3068617405,#clan id
                2: 1,
                4: str("caYsdb7hRrmuiqXs7WfDdQ"),#clan key
            }
        }
        packet = create_protobuf_packet(fields)
        packet = packet.hex()
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""[C][B]
مرحبًا بكم في كلان [00ff00]تناهيدㅤㅤㅤོ [ffffff]

[C][B]العبوا حروب الرابطه وجمعوا كلوري عبود يحبكم 
إذا أردت معرفة الأوامر، فقط أرسل: [00ff00]🗿/🗿help🗿[ffffff]

[C][B]
تنبيه! البوت لا يشتغل إلا لمن لديهم تاغ الكلان: [00ff00]—͟͞͞★[ffffff]
"""
                            )
                        )
                else:
...
####################################
            ...
            if "1200" in data.hex()[0:4] and b"/type/" in data:
                try:
                    raw_message = data.decode('utf-8', errors='ignore')
                    cleaned_message = raw_message.replace('\x00', '').strip()
                    
                    import re
                    id_match = re.search(r'/type/(\S+)', cleaned_message)
                    if not id_match:
                        return

                    full_word = id_match.group(1)
                    ...
                    if "★" not in sender_name:
                        ...
                        for i in range(1, len(full_word)+1):
                            colored_text = "".join([generate_random_color() + ch for ch in full_word[:i]])
                            clients.send(self.GenResponsMsg(colored_text))
                            sleep(0.5)
                except Exception as e:
